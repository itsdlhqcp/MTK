...
const StarRating = ({ rating, onRatingChange }) => {
  ...
  style={{ 
    ...
    width: '100%', 
    marginRight: wp(7), // Add marginRight for spacing
    paddingHorizontal: wp(10), // Responsive padding using wp() helper
...
const RatingBottomSheet = ({ visible, onClose, onSubmit, item }) => {
  ...
  // Calculate responsive snap points based on screen size
  const snapPoints = useMemo(() => {
    // Define different snap points for different screen sizes
    const getSnapPoint = () => {
      // For very small screens (height < 600px) - like iPhone SE
      if (screenHeight < 600) {
        return '79%';
      }
      // For small screens (height < 700px) - like iPhone 8
      else if (screenHeight < 700) {
        return '74%';
      }
      // For medium screens (height < 800px) - like iPhone 11
      else if (screenHeight < 800) {
        return '69%';
      }
      // For large screens (height < 900px) - like iPhone 14 Plus
      else if (screenHeight < 900) {
        return '64%';
      }
      // For extra large screens (height >= 900px) - like iPad
      else {
        return '59%';
      }
    };
