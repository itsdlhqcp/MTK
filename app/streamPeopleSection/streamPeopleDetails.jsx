...
const StreamPeopleDetails = () => {
    ...
    useEffect(() => {
        ...
        channelsRef.current = subscribeToChanges();
        
        return () => {
            // Clean up using stored channels
            if (channelsRef.current) {
                supabase.removeChannel(channelsRef.current.reviewChannel);
                supabase.removeChannel(channelsRef.current.replyChannel);
                supabase.removeChannel(channelsRef.current.areviewChannel);
                supabase.removeChannel(channelsRef.current.areplyChannel);
            }
        };
    }, [streamId]);
    ...
    const subscribeToChanges = () => {
        ...
        // Subscribe to AA reviews
        const areviewChannel = supabase
            .channel('preview')
            .on('postgres_changes', 
        ...
        // Subscribe to AA replies
        const areplyChannel = supabase
