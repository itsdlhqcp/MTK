...
const twistDetails = () => {
    ...
    useEffect(() => {
        let commentChannel = supabase
            .channel('tcomments')
            .on('postgres_changes', {event: 'INSERT', schema: 'public', table: 'tcomments', filter: `twistId=eq.${postId}`}, handleNewComment)
            .subscribe();

        // New likes subscription
        let likesChannel = supabase
            .channel('twistLikes')
            .on('postgres_changes', {event: '*', schema: 'public', table: 'twistLikes', filter: `twistId=eq.${postId}`}, handleLikeChange)
            .subscribe();
            ...
            if(res.success){
                if(user.id !== post.userId){
                    ...
                    let notify = {
                        ...
                        title: 'commented on your twist', 
                        data: JSON.stringify({postId: post.id, tcommentId: res?.data?.id})  // (also tru commentId) -if notification not poiinted coreecttly modify this
                    }
